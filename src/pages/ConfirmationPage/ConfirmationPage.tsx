      ...
      <PageShell onNavigate={onNavigate} accountLabel={user ? `Olá, ${user.name.split(' ')[0]}` : 'Entrar'}>
        <section className={styles.emptyState}>
          <div>
            ...
            <button type="button" onClick={() => onNavigate('/#reserva')}>Fazer uma reserva</button>
          </div>
        </section>
      </PageShell>
    );
      ...
      <section className={styles.section} aria-labelledby="confirmation-title">
        <div className={styles.successCard}>
          ...
          <p className={styles.lead}>
            A confirmação seria enviada para <strong>{confirmation.customer.email}</strong>. Guarde a referência abaixo.
          </p>
        ...
        <div className={styles.detailsLayout}>
          ...
          <aside className={styles.bookingDetails}>
            ...
            <dl>
              ...
              <div><dt>Passageiro</dt><dd>{confirmation.customer.fullName}</dd></div>
              <div><dt>Pagamento</dt><dd>{confirmation.customer.paymentMethod === 'cash' ? 'Dinheiro no veículo' : 'Cartão no veículo'}</dd></div>
              <div><dt>Total estimado</dt><dd className={styles.total}>{formatCurrency(confirmation.total)}</dd></div>
            </dl>
        ...
        <div className={styles.actions}>
          <button type="button" className={styles.primaryButton} onClick={onNewBooking}>Fazer nova reserva</button>
          <button type="button" className={styles.secondaryButton} onClick={() => onNavigate('/')}>Voltar ao início</button>
        </div>
      </section>
    </PageShell>
  );
