...
interface CheckoutPageProps {
  ...
  serverPricing?: PricingBreakdown | null;
  onNavigate: NavigateFunction;
}

export function CheckoutPage({ user, booking, service, vehicle, onSubmit, onNavigate, serverPricing }: CheckoutPageProps) {
  const { values, errors, isSubmitting, submitError, updateField, handleInputChange, handleNotesChange, handleSubmit } =
  ...
  if (!pricing) return <PageShell onNavigate={onNavigate}><section className={styles.emptyState} role="status"><div><h1>A obter o orçamento…</h1><button onClick={() => window.location.reload()}>Atualizar orçamento</button></div></section></PageShell>;
    ...
    >
      <section className={styles.pageHero}>
        <div>
          ...
          <span>Pagamento à chegada, em dinheiro ou cartão. A reserva fica pendente de confirmação.</span>
        </div>
      </section>
      ...
      <div className={styles.content}>
        ...
        <div className={styles.layout}>
          <form className={styles.form} onSubmit={handleSubmit} noValidate>
            {submitError && <p role="alert">{submitError} <button type="button" onClick={() => window.location.reload()}>Atualizar orçamento</button></p>}
